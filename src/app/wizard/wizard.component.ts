...
export class WizardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.daysInMonth = Array(this.getDaysInMonth())
      .fill(0)
      .map((x, i) => i + 1);
  }
  ...
  getDaysInMonth(): number {
    return new Date(
      this.basicForm.value.coverageStart.year,
      this.basicForm.value.coverageStart.month,
      0
    ).getDate();
  }
  updateDaysInMonth(): void {
    this.daysInMonth = Array(this.getDaysInMonth())
      .fill(0)
      .map((x, i) => i + 1);
    if (this.basicForm.get('coverageStart.day').value > this.getDaysInMonth()) {
      this.basicForm.get('coverageStart').patchValue({ day: null });
    }
  }
